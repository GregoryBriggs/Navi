...
class Feedback():   
    ...
    def processOrientation(self, tup_ax_ay_under):
        ...
        print("Value for eval: ", tup_ax_ay_under)
        ...
        if (len(self.new_path) > 1):
            ...
            if (self.new_path[0], self.new_path[1]) in self.graph:
                ...
                # compare magnet reading to when next node is in the negative y-direction
                    ...
                    if ax_under > -0.125 and ax_under < 0.125 and ay_under > 0:
                        self.turn = self.turn_around
                    elif ax_under > 0 and ay_under > 0 :
                        self.turn = self.right
                    elif ax_under < 0 and ay_under > 0: 
                ...
                # compare magnet reading to when next node is in the positive x-direction
                    ...
                    if ay_under > -0.125 and ay_under < 0.125 and ax_under < 0:
                        self.turn = self.turn_around
...
#
    ...
    def normalizeProjection(self, tup_x_y):
        ...
        norm_xy_and_r_theta = [0.0,0.0,0.0,0.0]
        ...
        norm_xy_and_r_theta[3] = math.atan2( tup_x_y[1] , tup_x_y[0])      # returned in radians from -pi to pi
...
while True:
    ...
    feedback.processOrientation(imu.normalizeProjection(imu.changeOfBasis(A_x, A_y, list(imu.readIMU(getMag, noAcc, noGyro, noMagMag, noSpeed)))))
